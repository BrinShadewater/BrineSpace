...
ROOT = HERE.parents[3]
OUT = ROOT / 'output/branforth-locker-identity-2026-09-22'
...
def main():
    ...
    idle = Image.open(ROOT / 'character/chief-engineer-branforth-v2/frames/bare/idle-east/000.png').convert('RGBA')
    geared = Image.open(ROOT / 'character/chief-engineer-branforth-v2/frames/helmet/idle-east/000.png').convert('RGBA')
    palette_pixels = []
    ...
    scale = 148 / 410
    poses, records = [], []
    # Remove erroneous duplicate faces from carried/raised helmet interiors.
    # Polygons use cell-local source coordinates; the raw source stays untouched.
    cleanup = {2:[(255,250),(285,247),(288,282),(254,279)],
               3:[(228,215),(249,212),(253,244),(227,240)],
               4:[(265,50),(293,48),(297,84),(264,79)]}
    for index in range(8):
        box = (index % 4 * 384, index // 4 * 512, index % 4 * 384 + 384, index // 4 * 512 + 512)
        cell = raw.crop(box)
        if index in cleanup:
            ImageDraw.Draw(cell).polygon(cleanup[index], fill=(20,24,25,255))
        a = np.array(cell)
        ground = cell.getbbox()[3]
        ys, xs = np.where(a[ground-17:ground, :, 3] > 0)
        ...
        paste = (round(92 - foot_x * scale), round(196 - ground * scale))
        frame.alpha_composite(mapped, paste)
        poses.append(frame)
        records.append({'cell': box, 'foot': [foot_x, ground], 'paste': paste})
    ...
    recipe = {'source_sha256': hashlib.sha256(source.read_bytes()).hexdigest(),
              ...
              'scale':scale, 'alpha_threshold':128, 'frames':records, 'cleanup_polygons':cleanup,
