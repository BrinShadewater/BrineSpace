"""Package reference-generated Veld rows; reuse Bill's deterministic registration."""
...
def main():
    ...
    manifest = dict(name='dr-veld-v1', frameWidth=SIZE, frameHeight=SIZE, pivot=[46, 86], standingHeight=74,
                    ...
                    actionMeaning={'interact':'scanner reading','repair':'sample inspection'}, cleanup=report, states=[])
    ...
    resource = [f'[gd_resource type="SpriteFrames" load_steps={len(states)*6+2} format=3]', '',
                '[ext_resource type="Texture2D" path="res://character/dr-veld-v1/final/spritesheet.png" id="1"]','']
    ...
    resource += [']']
    (ROOT/'final/dr-veld.tres').write_text('\n'.join(resource)+'\n')
    if not partial:
        chain = manifest['actionChain']
        packing.save_gif([frame for state in chain for frame in animations[state]], ROOT/'qa/science-sequence.gif',
