...
func _ready() -> void:
	super._ready()
	var image:=Image.new()
	if image.load_png_from_buffer(FileAccess.get_file_as_bytes("res://rooms/underwater/batch-two/clone_lab-source-v1.png")) != OK: push_error("Failed to load image (rooms/underwater/batch-two/clone_lab_view.gd:9)")
