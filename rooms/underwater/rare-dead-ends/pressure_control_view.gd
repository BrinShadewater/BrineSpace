...
func _ready() -> void:
	...
	preload("res://scripts/safe_image.gd").load_png(flush_image, "res://legacy/default/assets/rooms/pressure-control/source/pressure-fitted.png")
	...
	dressing=Dressing.new(self,"res://rooms/underwater/rare-dead-ends/pressure-composition-v2.json")
	rebuild()
...
func configure_embedded(q: int, open_sides: Array, running: bool, time_seconds: float, omitted_sides: Array = []) -> void:
	...
	if quarter==0 and flush_texture!=null:
		...
		for prop in props:
			...
			if spec.get("centerpiece",false) or spec.get("authored_anchor",false): authored.append(prop)
		props=[
