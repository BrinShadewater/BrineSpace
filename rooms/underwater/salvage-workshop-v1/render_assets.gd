...
func run() -> void:
	...
	for id in ["bench","tote"]:
		var base: String="res://rooms/underwater/salvage-workshop-v1/"+id
		var data: Dictionary=JSON.parse_string(FileAccess.get_file_as_string(base+".json"))
		var im:=Image.new()
		if im.load_png_from_buffer(FileAccess.get_file_as_bytes(data.source)) != OK: push_error("Failed to load image (rooms/underwater/salvage-workshop-v1/render_assets.gd)")
		...
		sprite.queue_free()
		await process_frame
	print("WORKSHOP CUTOUTS PASS: two native transparent renders")
	quit()
