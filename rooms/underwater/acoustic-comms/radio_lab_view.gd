...
func _ready() -> void:
	super._ready()
	var image := Image.new()
	preload("res://scripts/safe_image.gd").load_png(image, "res://legacy/default/assets/rooms/radio-lab/source/overhead.png")
	life_texture=ImageTexture.create_from_image(image)
	# Keep structural wall samples on the original atlas; repaint equipment only.
	preload("res://scripts/safe_image.gd").load_png(image, "res://legacy/default/assets/rooms/radio-lab/source/radio-equipment-v2.png")
