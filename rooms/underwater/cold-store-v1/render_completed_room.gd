...
class Preview extends Node2D:
	...
	func _draw() -> void:
		...
		draw_set_transform(origin,0,Vector2.ONE*zoom)
		preload("res://rooms/whole-room/north_wall.gd").draw_into(self,"cold_store",Vector2i.ZERO,false,false,room)
...
func run() -> void:
	...
	preload("res://scripts/room_layout_store.gd").path="res://output/cold-store-preview-layouts.json"
	var room:=View.new()
	room.embedded=true
	room.hide()
	root.add_child(room)
	...
	preview.room=room
	var image:=Image.new()
	preload("res://scripts/safe_image.gd").load_png(image, "res://legacy/default/rooms/foundation-v1/foundation-silt-v1.png")
	preview.foundation=ImageTexture.create_from_image(image)
	root.add_child(preview)
	await process_frame
	await RenderingServer.frame_post_draw
	assert(root.get_texture().get_image().save_png("res://legacy/default/rooms/underwater/cold-store-v1/completed-room.png")==OK)
	room.configure_embedded(0,[0,2],true,0)
	...
	for path in ["res://rooms/underwater/cold-store-v1/cold_store_view.gd","res://assets/rooms/observation-room/office/render_completed_room.gd"]:
	...
	print("COMPLETED COLD STORE PREVIEW PASS: two props, native riser/foundation, workbench access and sprite bounds")
	quit()
