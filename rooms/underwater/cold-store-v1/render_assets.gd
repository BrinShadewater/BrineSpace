...
func run() -> void:
	...
	for id in ["fridge-v2","rack-v2"]:
		...
		var im:=Image.new()
		if im.load_png_from_buffer(FileAccess.get_file_as_bytes(data.source)) != OK: push_error("Failed to load image (rooms/underwater/cold-store-v1/render_assets.gd)")
