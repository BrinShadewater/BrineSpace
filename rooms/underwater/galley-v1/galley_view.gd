...
func _ready() -> void:
	...
	for id in ["kitchen","serving"]:
		var image:=Image.new()
		if image.load_png_from_buffer(FileAccess.get_file_as_bytes("res://rooms/underwater/galley-v1/"+id+".png")) != OK: push_error("Failed to load image (rooms/underwater/galley-v1/galley_view.gd)")
