...
class Preview extends Node2D:
	...
	func _draw() -> void:
		...
		draw_set_transform(origin,0,Vector2.ONE*zoom)
		preload("res://rooms/whole-room/north_wall.gd").draw_into(self,"galley",Vector2i.ZERO,false,false,room)
...
func run() -> void:
	...
	preload("res://scripts/room_layout_store.gd").path="res://output/galley-preview-layouts.json"
	var room:=View.new()
	room.embedded=true
	room.hide()
	root.add_child(room)
	...
	preview.room=room
	var image:=Image.new()
	if image.load_png_from_buffer(FileAccess.get_file_as_bytes("res://rooms/foundation-v1/foundation-silt-v1.png")) != OK: push_error("Failed to load image (rooms/underwater/galley-v1/render_completed_room.gd)")
	preview.foundation=ImageTexture.create_from_image(image)
	root.add_child(preview)
	await process_frame
	await RenderingServer.frame_post_draw
	assert(root.get_texture().get_image().save_png("res://rooms/underwater/galley-v1/completed-room.png")==OK)
	...
	print("COMPLETED GALLEY PREVIEW PASS: two props, native riser/foundation, workbench access and sprite bounds")
	quit()
