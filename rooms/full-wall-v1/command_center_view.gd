extends "res://rooms/production-ten/command_center_view.gd"
## Balanced full-wall installation; original view is the reversible baseline.
var full_wall = preload("res://rooms/full-wall-v1/split_wall_prop.gd").new("command-wall")

func configure_embedded(q: int, open_sides: Array, running: bool, time_seconds: float, omitted_sides: Array = []) -> void:
	super.configure_embedded(q,open_sides,running,time_seconds,omitted_sides)
	full_wall.apply(self)

func prop_visual_bounds(prop: Dictionary) -> Rect2:
	if prop.get("library_asset",false): return preload("res://scripts/room_asset_library.gd").bounds(prop)
	if full_wall.owns(prop): return full_wall.bounds(prop)
	return super.prop_visual_bounds(prop)

func draw_registered_prop(prop: Dictionary) -> void:
	if full_wall.owns(prop):
		full_wall.draw(self,prop)
		if operating:
			var box: Rect2=full_wall.bounds(prop)
			var ops: bool=prop.id.ends_with("_operations")
			var at: Vector2
			match prop.side_view:
				"north": at=Vector2(0.53,0.34) if ops else Vector2(0.43,0.47)
				"south": at=Vector2(0.53,0.62) if ops else Vector2(0.36,0.60)
				"west": at=Vector2(0.42,0.39) if ops else Vector2(0.55,0.23)
				_: at=Vector2(0.58,0.39) if ops else Vector2(0.45,0.23)
			var center: Vector2=box.position+at*box.size
			var radius: float=minf(box.size.x,box.size.y)*0.085
			var end: Vector2=center+Vector2(cos(machine_clock*1.4),sin(machine_clock*1.4))*radius if ops else center+Vector2(radius*(0.4+0.6*sin(machine_clock*2)),0)
			painter.draw_line(center,end,Color("6798a5"),0.8,true)
		return
	super.draw_registered_prop(prop)

func is_animated_prop(prop: Dictionary) -> bool:
	if full_wall.owns(prop): return true
	return super.is_animated_prop(prop)

