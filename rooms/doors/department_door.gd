...
const VARIANTS := ["bio","life-support","engineering","generic"]
const APERTURE := 72.0
...
static func department(room: Dictionary) -> String:
	match str(room.get("id","")):
		"mycelium_nursery", "hydroponics_bay": return "bio"
		"life_support": return "life-support"
		"reactor": return "engineering"
	return "generic"
...
static func corridor_parts(frame: int, vertical: bool, variant: String) -> Array:
	...
	for source_part in side_parts(frame,variant):
		...
		if part.floor or part.get("leaf",false):
			...
			part.depth = 0.0
			part.erase("leaf") # Front surface uses the same restrained trim, no side-only edge lines.
		else:
...
static func side_parts(frame: int, variant: String) -> Array:
	...
	for north in [true,false]:
		...
		cap["wall"] = true
		out.append(cap)
		out.append(solid_piece(Rect2(-4,ground_y,8,10),trim,depth))
		out.append(solid_piece(Rect2(-2,ground_y+2,4,6),Color("29353a"),depth))
...
static func draw_piece(canvas: CanvasItem, materials: Dictionary, variant: String, part: Dictionary, level: float) -> void:
	if part.get("wall",false):
		if materials.has("wall"): canvas.draw_texture_rect_region(materials.wall,part.rect,part.source)
		else: canvas.draw_rect(part.rect,Color("ddd8bf"))
		return
	if part.has("color"):
		var color: Color = part.color
		if part.get("emissive",false): color = Color("29353a").lerp(color,clampf(level,0,1))
