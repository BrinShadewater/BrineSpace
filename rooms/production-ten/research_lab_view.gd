...
func _ready() -> void:
	super._ready()
	var image := Image.new()
	if image.load_png_from_buffer(FileAccess.get_file_as_bytes("res://rooms/production-ten/research_lab-source-v1.png")) != OK: push_error("Failed to load image (rooms/production-ten/research_lab_view.gd:12)")
