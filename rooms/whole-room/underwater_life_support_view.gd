...
func _ready() -> void:
	super._ready()
	var image := Image.new()
	if image.load_png_from_buffer(FileAccess.get_file_as_bytes("res://rooms/whole-room/life-support-underwater-source-v1.png")) != OK: push_error("Failed to load image (rooms/whole-room/underwater_life_support_view.gd:12)")
