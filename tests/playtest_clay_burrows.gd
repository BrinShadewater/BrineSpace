...
class Sheet extends Control:
	...
	func _draw() -> void:
		...
		for panel in range(2):
			...
			for i in range(3):
				var side: float=[45.0,54.0,63.0][i]
				var at:=origin+Vector2(80+i*210,250)
				draw_texture_rect(prop,Rect2(at,Vector2.ONE*side),false,Color(.49,.58,.57,.9))
				draw_string(ThemeDB.fallback_font,at+Vector2(0,160),["0.25 cell","0.30 cell","0.35 cell"][i],HORIZONTAL_ALIGNMENT_LEFT,-1,20)
...
func run() -> void:
	...
	for path in ["res://legacy/default/assets/environment/clay-silt-v1/clay-silt-ground-v1.png","res://legacy/default/assets/environment/volcanic-ash-v1/ash-ground-v1.png"]:
		var source:=Image.new()
		assert(source.load(path)==OK)
		sheet.grounds.append(ImageTexture.create_from_image(source))
	var prop:=Image.new()
	assert(prop.load("res://legacy/default/assets/environment/clay-silt-v1/burrow-mouths-v1.png")==OK)
	...
	DirAccess.make_dir_recursive_absolute("res://output/clay-burrows-v1")
	assert(frame.save_png("res://output/clay-burrows-v1/terrain-scale.png")==OK)
	print("BURROW STUDY PASS: two grounds, three cell widths, alpha source, 1600x900 capture")
	quit()
