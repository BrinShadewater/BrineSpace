extends SceneTree
const ROOMS := ["med_bay","research_lab","command_center"]
var game
...
func run() -> void:
	...
	await settle()
	DirAccess.make_dir_recursive_absolute("res://output/room-character-v1")
	for q in range(4):
		var width := 1600
		...
		for room_id in ROOMS:
			...
			assert(frame.save_png("res://output/room-character-v1/%s-%d.png" % [room_id,q])==OK)
