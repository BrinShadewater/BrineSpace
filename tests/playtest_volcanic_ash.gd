extends SceneTree
const OUT := "res://output/volcanic-ash-v1/"
...
func run() -> void:
	...
	var paths := ["res://assets/environment/volcanic-ash-v1/ash-ground-v1.png","res://assets/environment/service-wreckage-v1/collapsed-support-v1.png","res://assets/environment/shell-shoal-v1/limestone-cobbles-v1.png"]
	var textures: Array[Texture2D] = []
	for path in paths:
		var source := Image.new()
		if source.load(path) != OK: push_error("Failed to load image (tests/playtest_volcanic_ash.gd:43)")
	...
	if porous_image.load("res://assets/environment/volcanic-ash-v1/porous-rocks-v1.png") != OK: push_error("Failed to load image (tests/playtest_volcanic_ash.gd:49)")
	...
	assert(frame.save_png(OUT+"porous-rock-contrast.png")==OK)
	print("VOLCANIC ASH STUDY PASS: four comparison panels; four sources loaded; 1600x900 capture")
	quit()
