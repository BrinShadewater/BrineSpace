extends SceneTree
const View := preload("res://assets/environment/sea-lettuce-v1/sea_lettuce_view.gd")
const OUT := "res://output/sea-lettuce-station-v1/"
var game
...
class Sheet extends Control:
	...
	func _draw() -> void:
		...
		draw_string(font,Vector2(30,44),"BRINE / SEA LETTUCE",HORIZONTAL_ALIGNMENT_LEFT,-1,26,Color("b7cec5"))
		...
		for id in View.SOURCES:
			...
			i += 1
		draw_string(font,Vector2(30,780),"Sea lettuce source sheet. Decorative ground and debris; no resource or clearance mechanics.",HORIZONTAL_ALIGNMENT_LEFT,-1,20,Color("839f9d"))
...
func run() -> void:
	...
	game.meta.save_path = "user://sea_lettuce_%d.meta" % OS.get_process_id()
	game.run_save_path = "user://sea_lettuce_%d.loop" % OS.get_process_id()
	root.add_child(game)
	current_scene = game
	...
	assert(game.occupied==occupancy and game.resources==resources,"Rendering sea lettuce changes no gameplay state")
	assert(game.grid_view.seabed_background.sea_lettuce.textures.size()==1)
	...
	print("SEA LETTUCE STATION PASS: one texture, three verified resolutions, unchanged occupancy and resources")
	quit()
