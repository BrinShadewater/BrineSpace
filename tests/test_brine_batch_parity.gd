...
func mode(cached: bool) -> void:
	preload("res://rooms/whole-room/room_floor.gd").batch_lines = cached
	game.grid_view.brine_view.retain_brine_parts = cached
...
func changed_state(label: String) -> void:
	...
	await settle()
	root.get_texture().get_image().save_png("res://output/brine-batch-parity-%s-cached.png" % label)
	mode(false)
	await settle()
	root.get_texture().get_image().save_png("res://output/brine-batch-parity-%s-direct.png" % label)
	mode(true)
	await settle()
func run() -> void:
	preload("res://scripts/title_settings.gd").save_path = "user://brine_batch_parity.cfg"
	var main = load("res://scenes/main.tscn")
	game = main.instantiate()
	game.meta.save_path = "user://brine_batch_parity.meta"
	game.run_save_path = "user://brine_batch_parity.loop"
	root.add_child(game)
	current_scene = game
	...
	for q in range(4):
		...
		for enabled in [false,true]:
			mode(enabled)
			await settle()
			root.get_texture().get_image().save_png("res://output/brine-batch-parity-q%d-%s.png" % [q,"cached" if enabled else "direct"])
	...
	for enabled in [false,true]:
		mode(enabled)
		await settle()
		root.get_texture().get_image().save_png("res://output/brine-batch-parity-close-%s.png" % ("cached" if enabled else "direct"))
	var static_before := 0
	var live_before := 0
	...
	for q in range(4):
		...
		for tick in range(3):
			...
			for enabled in [true,false]:
				mode(enabled)
				await settle()
				root.get_texture().get_image().save_png("res://output/brine-batch-parity-motion-%d-%d-%s.png" % [q,tick,"cached" if enabled else "direct"])
	mode(true)
	await settle()
	...
	for frame in range(160):
		...
		if frame in [19,59,99,159]:
			for enabled in [true,false]:
				mode(enabled)
				await settle()
				root.get_texture().get_image().save_png("res://output/brine-batch-parity-busy-%d-%s.png" % [frame,"cached" if enabled else "direct"])
			mode(true)
