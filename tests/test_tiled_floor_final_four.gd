...
const IDS=["observation_room", "salvage_workshop", "galley", "cold_store"]
func _init() -> void: call_deferred("run")
func frame() -> void: await process_frame; RenderingServer.force_draw()
func run() -> void:
	DirAccess.make_dir_recursive_absolute("res://output/tiled-floor-final-four")
	...
	for id in IDS:
		...
		root.get_texture().get_image().save_png("res://output/tiled-floor-final-four/"+id+".png")
		# Persist original before moving on; prior orientation drafts remain independent.
		e.save_layout()
	e.close_editor(); await process_frame
	var file:=FileAccess.open("res://output/tiled-floor-final-four/parity.json",FileAccess.WRITE); file.store_string(JSON.stringify(results,"\t")); file.close()
	print("TILED FLOOR BATCH PASS: four identities, 16 default comparisons, material cache isolation, editor finish/undo/save/reload/reset")
	quit()
