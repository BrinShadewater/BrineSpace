...
func run() -> void:
	...
	for path in ["res://assets/environment/clay-silt-v1/clay-silt-ground-v1.png","res://assets/environment/volcanic-ash-v1/ash-ground-v1.png"]:
		...
		if source.load(path) != OK: push_error("Failed to load image (tests/playtest_driftwood.gd:28)")
		sheet.grounds.append(ImageTexture.create_from_image(source))
	var prop:=Image.new()
	if prop.load("res://assets/environment/driftwood-v1/waterlogged-timber-v1.png") != OK: push_error("Failed to load image (tests/playtest_driftwood.gd:31)")
