extends SceneTree
const View := preload("res://assets/environment/ripple-sand-v1/ripple_sand_view.gd")
const OUT := "res://output/ripple-sand-station-v1/"
var game
...
class Sheet extends Control:
	...
	func _draw() -> void:
		...
		draw_string(font,Vector2(30,44),"BRINE / RIPPLE SAND",HORIZONTAL_ALIGNMENT_LEFT,-1,26,Color("b7cec5"))
		draw_string(font,Vector2(30,80),"Original source colors / opaque terrain canvas",HORIZONTAL_ALIGNMENT_LEFT,-1,19,Color("839f9d"))
		var i := 0
		for id in View.SOURCES:
			...
			i += 1
		draw_string(font,Vector2(30,780),"Ripple sand source sheet. Decorative ground and debris; no resource or clearance mechanics.",HORIZONTAL_ALIGNMENT_LEFT,-1,20,Color("839f9d"))
...
func run() -> void:
	...
	game.meta.save_path = "user://ripple_sand_%d.meta" % OS.get_process_id()
	game.run_save_path = "user://ripple_sand_%d.loop" % OS.get_process_id()
	root.add_child(game)
	current_scene = game
	...
	assert(game.occupied==occupancy and game.resources==resources,"Rendering ripple_sand changes no gameplay state")
	assert(game.grid_view.seabed_background.ripple_sand.textures.size()==1)
	...
	print("RIPPLE SAND STATION PASS: one texture, three verified resolutions, unchanged occupancy and resources")
	quit()
