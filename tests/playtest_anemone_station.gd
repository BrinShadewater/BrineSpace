extends SceneTree
const View := preload("res://assets/environment/anemone-v1/anemone_view.gd")
const OUT := "res://output/anemone-station-v1/"
var game
...
class Sheet extends Control:
	...
	func _draw() -> void:
		...
		draw_string(font,Vector2(30,44),"BRINE / ANEMONE",HORIZONTAL_ALIGNMENT_LEFT,-1,26,Color("b7cec5"))
		...
		for id in View.SOURCES:
			...
			i += 1
		draw_string(font,Vector2(30,780),"Anemones source sheet. Decorative ground and debris; no resource or clearance mechanics.",HORIZONTAL_ALIGNMENT_LEFT,-1,20,Color("839f9d"))
...
func run() -> void:
	...
	game.meta.save_path = "user://anemone_%d.meta" % OS.get_process_id()
	game.run_save_path = "user://anemone_%d.loop" % OS.get_process_id()
	root.add_child(game)
	current_scene = game
	...
	assert(game.occupied==occupancy and game.resources==resources,"Rendering anemones changes no gameplay state")
	assert(game.grid_view.seabed_background.anemone.textures.size()==1)
	...
	print("ANEMONE STATION PASS: one texture, three verified resolutions, unchanged occupancy and resources")
	quit()
