...
func verify_motion_and_routes() -> void:
	...
	for q in range(4):
		...
		for side in range(4):
			expect(Geometry.has_port(view.layout[0],side)==actual.has(names[side]),"Medical Office canonical straight sockets")
			if actual.has(names[side]):
				for step in range(101): expect(Geometry.can_stand(Vector2(Geometry.DIRS[side])*181*step/100.0,view.layout,view.props,view.edges),"Medical Office socket route")
		for state in cases:
			...
			expect(game.powered_room_cells.has(cell)==state.active,"Medical Office economy operation: "+state.name)
			expect(game.grid_view._room_light_target(game.occupied[cell])==state.light,"Medical Office independent lighting: "+state.name)
			game._refresh_all()
			game.visual_time_seconds=0.2
			await capture("med-office-economy-q%d-%s-a"%[q,state.name])
			var before: Array=[]
			for prop in view.props: before.append(machine_pixels(cell,prop))
			game.visual_time_seconds=1.1
			await capture("med-office-economy-q%d-%s-b"%[q,state.name])
			for i in range(view.props.size()): expect((before[i]!=machine_pixels(cell,view.props[i]))==(state.active and view.is_animated_prop(view.props[i])),"Medical Office host operation: "+str(view.props[i].id))
	print("MED OFFICE ECONOMY PASS: four rotations, four actual states including restoration, four hosts and 808 socket samples")
