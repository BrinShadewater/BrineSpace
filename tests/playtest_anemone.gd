...
class Sheet extends Control:
	...
	func _draw() -> void:
		...
		var names := ["Anemones","Urchin","Red algae"]
...
func run() -> void:
	...
	for path in ["res://assets/environment/anemone-v1/low-anemones-v1.png","res://assets/environment/urchin-v1/short-spine-urchin-v1.png","res://assets/environment/red-algae-v1/red-algae-tuft-v1.png"]:
		...
		if source.load(path) != OK: push_error("Failed to load image (tests/playtest_anemone.gd:31)")
	...
	DirAccess.make_dir_recursive_absolute("res://output/anemone-v1")
	assert(frame.save_png("res://output/anemone-v1/plant-scale.png")==OK)
	print("ANEMONE STUDY PASS: three alpha sources; three cell widths; source and runtime tint; 1600x900")
	quit()
