...
const OUT = "res://output/copied-prop-animation-2026-09-21"
class Direct extends Node2D:
	...
	func _draw():
		...
		for prop in room.props:
			var artwork=prop.duplicate()
			artwork.id=artwork.get("copy_source",artwork.id)
			room.draw_registered_prop(artwork)
var failures=0
...
func run():
	...
	var room=load("res://rooms/full-wall-v1/mining_drone_bay_view.gd").new()
	...
	for q in range(4):
		for operating in [true,false]:
			...
			var source=room.get_meta("layout_copy_sources")[q]["mining_tether"]
			var copy=source.duplicate(true)
			copy.id="copy/mining_tether#test";copy.copy_source="mining_tether"
			room.props=[copy]
			var queue:Array=[]
			...
			for tick in range(12):
				...
				await frame()
			var expected_live=12
			check(canvas.live_redraws-live_before==expected_live,"Copied tether keeps source animation: q%d operating=%s got=%d expected=%d"%[q,operating,canvas.live_redraws-live_before,expected_live])
			check(canvas.static_redraws==static_before,"No copied tether misclassified static q%d"%q)
	...
	print("COPIED PROP ANIMATION: four views, on/off, 12-frame redraw counts and exact direct parity; %d failures"%failures)
