...
func run() -> void:
	...
	DirAccess.make_dir_recursive_absolute("res://output/floor-kit-installed")
	var card:=Card.new()
	...
	for row in rows:
		...
		for q in range(4):
			...
			for live in [false,true]:
				...
				if q==0 and not live: assert(capture.save_png("res://output/floor-kit-installed/%s-offline.png"%row.id)==OK)
				if live: assert(capture.save_png("res://output/floor-kit-installed/%s-q%d.png"%[row.id,q])==OK)
				samples+=1
		if card.room!=null:
			...
			await process_frame
			await RenderingServer.frame_post_draw
			assert(root.get_texture().get_image().save_png("res://output/floor-kit-installed/%s-shared.png"%row.id)==OK)
			card.suspended=true
			card.queue_redraw()
			await process_frame
			await RenderingServer.frame_post_draw
			...
			for variant in range(3):
				...
				await process_frame
				await RenderingServer.frame_post_draw
				assert(root.get_texture().get_image().save_png("res://output/floor-kit-installed/%s-variant%d.png"%[row.id,variant])==OK)
			card.variant=0
		report.append({"id":row.id,"render_samples":samples,"view":row.view})
		print("DECORATION ROOM: ",row.id," rotations/state pass")
	var file:=FileAccess.open("res://output/floor-kit-installed/review.json",FileAccess.WRITE)
