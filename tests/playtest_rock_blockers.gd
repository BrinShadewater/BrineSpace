...
var game
const OUT := "res://output/rock-blockers-v1"
...
func frame_formation() -> void:
	...
	await settle()
	var target: Vector2 = Vector2(21,20)*game.get_cell_size()-game.grid_scroll.size*0.5
...
func run() -> void:
	...
	game.meta.save_path = "user://rock_native_%d.meta" % OS.get_process_id()
	game.run_save_path = "user://rock_native_%d.loop" % OS.get_process_id()
	root.add_child(game)
	current_scene = game
	...
	for cell in [Vector2i(17,19),Vector2i(23,18),Vector2i(23,19),Vector2i(23,20),Vector2i(24,20)]:
		game.wrecks[cell] = {"kind":"basalt","progress":0.0,"active":false,"cleared":false}
	for i in range(3):
		game._place_room(["reactor","corridor","crew_hab"][i],Vector2i(19+i,21),true)
	...
	print("ROCK NATIVE PASS: connected mass, isolated rock, L shelf, three resolutions, cutting motion, pause, notch/interior clearance and paid rebuild")
	quit()
