extends SceneTree
const View := preload("res://assets/environment/clay-silt-v1/clay_silt_view.gd")
const OUT := "res://output/clay-silt-station-v2/"
var game
...
class Sheet extends Control:
	...
	func _draw() -> void:
		...
		draw_string(font,Vector2(30,44),"BRINE / CLAY SILT",HORIZONTAL_ALIGNMENT_LEFT,-1,26,Color("b7cec5"))
		...
		for id in View.SOURCES:
			...
			i += 1
		draw_string(font,Vector2(30,780),"Clay silt source sheet. Decorative ground and debris; no resource or clearance mechanics.",HORIZONTAL_ALIGNMENT_LEFT,-1,20,Color("839f9d"))
...
func run() -> void:
	...
	game.meta.save_path = "user://clay_silt_%d.meta" % OS.get_process_id()
	game.run_save_path = "user://clay_silt_%d.loop" % OS.get_process_id()
	root.add_child(game)
	current_scene = game
	...
	assert(game.occupied==occupancy and game.resources==resources,"Rendering clay_silt changes no gameplay state")
	assert(game.grid_view.seabed_background.clay_silt.textures.size()==2)
	...
	print("CLAY SILT STATION PASS: two textures, three verified resolutions, unchanged occupancy and resources")
	quit()
