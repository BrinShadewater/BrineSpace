...
func mode(cached: bool) -> void:
	game.grid_view.retain_static_surfaces = cached
	for view in views: view.retain_shell_queues = cached
...
func changed_state(label: String) -> void:
	...
	await settle()
	root.get_texture().get_image().save_png("res://output/retained-lights-parity-%s-cached.png" % label)
	mode(false)
	await settle()
	root.get_texture().get_image().save_png("res://output/retained-lights-parity-%s-direct.png" % label)
	mode(true)
	await settle()
func run() -> void:
	preload("res://scripts/title_settings.gd").save_path = "user://retained_lights_parity.cfg"
	var main = load("res://scenes/main.tscn")
	game = main.instantiate()
	game.meta.save_path = "user://retained_lights_parity.meta"
	game.run_save_path = "user://retained_lights_parity.loop"
	root.add_child(game)
	current_scene = game
	...
	for q in range(4):
		...
		for enabled in [false,true]:
			mode(enabled)
			await settle()
			root.get_texture().get_image().save_png("res://output/retained-lights-parity-q%d-%s.png" % [q,"cached" if enabled else "direct"])
	...
	for enabled in [false,true]:
		mode(enabled)
		await settle()
		root.get_texture().get_image().save_png("res://output/retained-lights-parity-close-%s.png" % ("cached" if enabled else "direct"))
	var static_before := 0
	var live_before := 0
	...
	var retained_before := Vector2i(game.grid_view.door_rebuilds,game.grid_view.light_rebuilds)
	for frame in range(12):
		game.visual_time_seconds += 0.1
		game.grid_view.queue_redraw()
		await settle()
	assert(retained_before == Vector2i(game.grid_view.door_rebuilds,game.grid_view.light_rebuilds),"Stable doors and lights retain commands while animation advances")
	...
	for q in range(4):
		...
		for tick in range(3):
			...
			for enabled in [true,false]:
				mode(enabled)
				await settle()
				root.get_texture().get_image().save_png("res://output/retained-lights-parity-motion-%d-%d-%s.png" % [q,tick,"cached" if enabled else "direct"])
	mode(true)
	await settle()
	...
	for frame in range(160):
		...
		if frame in [19,59,99,159]:
			for enabled in [true,false]:
				mode(enabled)
				await settle()
				root.get_texture().get_image().save_png("res://output/retained-lights-parity-busy-%d-%s.png" % [frame,"cached" if enabled else "direct"])
			mode(true)
