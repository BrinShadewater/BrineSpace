...
func run() -> void:
	...
	for q in range(4):
		...
		check(view.layout[0].kind==1,"Medical Center NS layout")
			...
			for step in range(180):
				...
				for mark in marks:
					for point in mark:
						...
						samples+=1
	view.free()
	if not failed: print("MED CENTER REGISTRATION PASS: four rotations, full bounds/pieces, static supplies and ",samples," display-effect samples")
