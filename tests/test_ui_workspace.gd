...
func run() -> void:
	...
	game._refresh_all()
	check(game.hovered_card_id == "life_support", "Hovering a blueprint previews it in the inspector (owner playtest)")
	game._on_card_unhovered("life_support",card)
	game._refresh_all()
	check(game.preview_name_label.text == original, "Leaving a blueprint restores the clicked room")
	card.queue_free()
