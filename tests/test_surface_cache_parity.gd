...
func changed_state(label: String) -> void:
	...
	await settle()
	root.get_texture().get_image().save_png("res://output/surface-parity-%s-cached.png" % label)
	mode(false)
	await settle()
	root.get_texture().get_image().save_png("res://output/surface-parity-%s-direct.png" % label)
	mode(true)
	await settle()
func run() -> void:
	preload("res://scripts/title_settings.gd").save_path = "user://surface_parity.cfg"
	var main = load("res://scenes/main.tscn")
	game = main.instantiate()
	game.meta.save_path = "user://surface_parity.meta"
	game.run_save_path = "user://surface_parity.loop"
	root.add_child(game)
	current_scene = game
	...
	for q in range(4):
		...
		for enabled in [false,true]:
			mode(enabled)
			await settle()
			root.get_texture().get_image().save_png("res://output/surface-parity-q%d-%s.png" % [q,"cached" if enabled else "direct"])
	...
	for enabled in [false,true]:
		mode(enabled)
		await settle()
		root.get_texture().get_image().save_png("res://output/surface-parity-close-%s.png" % ("cached" if enabled else "direct"))
	...
	print("STATIC SURFACE REUSE PASS")
	...
	print("SURFACE PARITY CAPTURE %s: %d rooms, %d views, four rotations" % ["PASS" if failures==0 else "FAIL",game.placed_rooms.size(),views.size()])
	game.queue_free()
	await process_frame
	quit(failures)
