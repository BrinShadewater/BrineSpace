extends SceneTree
const View := preload("res://assets/environment/mooring-debris-v1/mooring_debris_view.gd")
const OUT := "res://output/mooring-debris-station-v1/"
var game
...
class Sheet extends Control:
	...
	func _draw() -> void:
		...
		draw_string(font,Vector2(30,44),"BRINE / MOORING DEBRIS",HORIZONTAL_ALIGNMENT_LEFT,-1,26,Color("b7cec5"))
...
func run() -> void:
	...
	game.meta.save_path = "user://mooring_debris_%d.meta" % OS.get_process_id()
	game.run_save_path = "user://mooring_debris_%d.loop" % OS.get_process_id()
	root.add_child(game)
	current_scene = game
	...
	assert(game.grid_view.seabed_background.mooring_debris.textures.size()==1)
	...
	print("MOORING DEBRIS STATION PASS: one texture, three verified resolutions, unchanged occupancy and resources")
	quit()
