extends SceneTree
const View := preload("res://assets/environment/cable-reel-v1/cable_reel_view.gd")
const OUT := "res://output/cable-reel-station-v1/"
var game
...
class Sheet extends Control:
	...
	func _draw() -> void:
		...
		draw_string(font,Vector2(30,44),"BRINE / CABLE REEL",HORIZONTAL_ALIGNMENT_LEFT,-1,26,Color("b7cec5"))
		...
		for id in View.SOURCES:
			...
			i += 1
		draw_string(font,Vector2(30,780),"Collapsed cable reel source sheet. Decorative ground and debris; no resource or clearance mechanics.",HORIZONTAL_ALIGNMENT_LEFT,-1,20,Color("839f9d"))
...
func run() -> void:
	...
	game.meta.save_path = "user://cable_reel_%d.meta" % OS.get_process_id()
	game.run_save_path = "user://cable_reel_%d.loop" % OS.get_process_id()
	root.add_child(game)
	current_scene = game
	...
	assert(game.occupied==occupancy and game.resources==resources,"Rendering cable_reel changes no gameplay state")
	assert(game.grid_view.seabed_background.cable_reel.textures.size()==1)
	...
	print("CABLE REEL STATION PASS: one texture, three verified resolutions, unchanged occupancy and resources")
	quit()
