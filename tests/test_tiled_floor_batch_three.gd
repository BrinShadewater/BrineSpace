...
const IDS=["hydroponics_bay", "mycelium_nursery", "tidal_condenser", "quarantine_cell", "cryo_chamber", "clone_lab", "med_bay", "storage_bay"]
func _init() -> void: call_deferred("run")
func frame() -> void: await process_frame; RenderingServer.force_draw()
func run() -> void:
	DirAccess.make_dir_recursive_absolute("res://output/tiled-floor-batch-three")
	...
	for id in IDS:
		...
		root.get_texture().get_image().save_png("res://output/tiled-floor-batch-three/"+id+".png")
		# Persist original before moving on; prior orientation drafts remain independent.
		e.save_layout()
	e.close_editor(); await process_frame
	var file:=FileAccess.open("res://output/tiled-floor-batch-three/parity.json",FileAccess.WRITE); file.store_string(JSON.stringify(results,"\t")); file.close()
	print("TILED FLOOR BATCH PASS: eight identities, 32 default comparisons, material cache isolation, editor finish/undo/save/reload/reset")
	quit()
