...
func run() -> void:
	...
	for path in paths:
		...
		if source.load(path) != OK: push_error("Failed to load image (tests/playtest_ripple_sand.gd:43)")
	...
	if porous_image.load("res://assets/environment/volcanic-ash-v1/porous-rocks-v1.png") != OK: push_error("Failed to load image (tests/playtest_ripple_sand.gd:49)")
