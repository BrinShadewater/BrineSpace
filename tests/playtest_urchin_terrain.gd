...
class Sheet extends Control:
	...
	func _draw() -> void:
		...
		for panel in range(2):
			...
			for i in range(3):
				var side: float=[27.0,36.0,45.0][i]
				var at:=origin+Vector2(80+i*210,250)
				draw_texture_rect(prop,Rect2(at,Vector2.ONE*side),false,Color(.57,.66,.63,.86))
				draw_string(ThemeDB.fallback_font,at+Vector2(0,160),["0.15 cell","0.20 cell","0.25 cell"][i],HORIZONTAL_ALIGNMENT_LEFT,-1,20)
...
func run() -> void:
	...
	for path in ["res://assets/environment/shell-shoal-v1/shell-hash-ground-v2.png","res://assets/environment/volcanic-ash-v1/ash-ground-v1.png"]:
		...
		if source.load(path) != OK: push_error("Failed to load image (tests/playtest_urchin_terrain.gd:28)")
		sheet.grounds.append(ImageTexture.create_from_image(source))
	var prop:=Image.new()
	if prop.load("res://assets/environment/urchin-v1/short-spine-urchin-v1.png") != OK: push_error("Failed to load image (tests/playtest_urchin_terrain.gd:31)")
	...
	DirAccess.make_dir_recursive_absolute("res://output/urchin-v1")
	assert(frame.save_png("res://output/urchin-v1/terrain-scale.png")==OK)
	print("URCHIN TERRAIN PASS: two grounds, three cell widths, alpha source, 1600x900 capture")
	quit()
