...
func run():
	...
	var stem="user://veld_bunk_%d"%OS.get_process_id()
	game.meta.save_path=stem+".meta";game.run_save_path=stem+".loop"
	game.meta.unlocked_architect_ids={"bill":true,"veld":true};game.meta.selected_architect="veld"
	...
	var actor=game.veld_npc;actor.rebuild(game)
	var station=actor.RoomActivity.stations(actor.geometry[HAB])[0]
	check(station.get("veld_bunk",false),"Compatible berth exposes the profile")
	...
	for other in [game.bill_npc,game.branforth_npc]:
		other.rebuild(game)
		check(not other.RoomActivity.stations(other.geometry[HAB]).any(func(item):return item.get("veld_bunk",false)),"Other cast do not inherit Veld's profile")
	...
	check(not actor.RoomActivity.stations(rejected).any(func(item):return item.get("veld_bunk",false)),"Mirrored bunk does not inherit unmirrored choreography")
	bunk.layout_flip=Vector2.ONE;bunk.rect.size*=1.05
	check(actor.RoomActivity.stations(rejected).any(func(item):return item.get("veld_bunk",false)),"Owner rescale keeps the profile (points are fractions of the art)")
	rejected=actor.geometry[HAB].duplicate(true);rejected.blockers.append(Rect2(station.point-Vector2.ONE*4,Vector2.ONE*8))
	check(not actor.RoomActivity.stations(rejected).any(func(item):return item.get("veld_bunk",false)),"Blocked contact rejected")
	var initial=actor.snapshot()
	DirAccess.make_dir_recursive_absolute("res://output/crew-activity/veld-bunk")
	check(is_equal_approx(game.grid_view.veld_player.cycle_seconds("bunk-enter-east"),1.84),"Manifest and actor durations agree")
	for equipped in [false,true]:
		for elapsed in [0.0,0.2,0.55,0.8,1.1,1.4,1.7]:
			...
			var offset=actor.observation_visual_offset();var texture=game.grid_view._get_veld_frame(game)
			...
			game.tick_timer.stop();game.set_process(false);actor=game.veld_npc;game.powered_room_cells[HAB]=true
			...
			check(game.grid_view._get_veld_frame(game).get_image().get_data()==pixels,"Restore preserves pose pixels")
			...
			check(root.get_texture().get_image().save_png("res://output/crew-activity/veld-bunk/restore-%s-%04d.png"%["helmet" if equipped else "bare",roundi(elapsed*1000)])==OK,"Capture live restored contact")
			game.paused=false
			...
			check(game.grid_view._get_veld_frame(game).get_image().get_data()==pixels,"Reverse interruption preserves pose")
			check(actor.valid_veld_snapshot(actor.snapshot()),"Interrupted rise validates")
	...
	var bad=initial.duplicate(true);bad.veld_bunk.rest=Vector2(INF,0)
	check(not actor.valid_veld_snapshot(bad),"Nonfinite contact rejected")
	bad=initial.duplicate(true);bad.veld_bunk.entry+=Vector2(5,0)
	check(not actor.valid_veld_snapshot(bad),"Detached saved entry rejected")
	bad=initial.duplicate(true);bad.direction=false
	check(not actor.valid_veld_snapshot(bad),"Malformed saved direction rejected")
	bad=initial.duplicate(true);bad.timer=1.85
	check(not actor.valid_veld_snapshot(bad),"Excess transition timer rejected")
	...
	print("VELD BUNK: normal chooser/arrival, fourteen bare/equipped disk restores, pause, reverse interruption, completion and invalid saves; %d failures"%failures)
	quit(failures)
