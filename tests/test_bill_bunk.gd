...
func run():
	...
	check(station.get("bill_bunk",false),"Compatible berth exposes the profile")
	...
	actor.decision_rng=RandomNumberGenerator.new();actor.decision_rng.seed=8923
	...
	actor.arrive()
	for other in [game.branforth_npc,game.veld_npc]:
		other.rebuild(game)
		check(not other.RoomActivity.stations(other.geometry[HAB]).any(func(item):return item.get("bill_bunk",false)),"Other cast do not inherit Bill's profile")
	...
	check(not actor.RoomActivity.stations(rejected).any(func(item):return item.get("bill_bunk",false)),"Mirrored bunk does not inherit unmirrored choreography")
	bunk.layout_flip=Vector2.ONE;bunk.rect.size*=1.05
	check(actor.RoomActivity.stations(rejected).any(func(item):return item.get("bill_bunk",false)),"Owner rescale keeps the profile (points are fractions of the art)")
	rejected=actor.geometry[HAB].duplicate(true);rejected.blockers.append(Rect2(station.point-Vector2.ONE*4,Vector2.ONE*8))
	check(not actor.RoomActivity.stations(rejected).any(func(item):return item.get("bill_bunk",false)),"Blocked contact rejected")
	var initial=actor.snapshot()
	DirAccess.make_dir_recursive_absolute("res://output/crew-activity/bill-bunk")
	check(is_equal_approx(game.grid_view.human_water_player.cycle_seconds("bunk-enter-east"),1.84),"Manifest and actor durations agree")
	for equipped in [false,true]:
		for elapsed in [0.0,0.2,0.55,0.8,1.1,1.4,1.7]:
			...
			var offset=actor.observation_visual_offset();var texture=game.grid_view._get_human_frame(game.get_test_walker_state(),game.get_test_walker_direction())
			...
			check(game.get_test_walker_state()==actor.animation_state(),"Paused rendering uses restored controller state")
			var frame_index=[0.0,0.2,0.55,0.8,1.1,1.4,1.7].find(elapsed)
			var expected=Image.load_from_file("res://character/major-bill-v3/supplemental/bunk-east/%s/%03d.png"%["helmet" if equipped else "bare",frame_index])
			check(pixels==expected.get_data(),"Renderer uses Bill source pixels for this contact phase")
			...
			game.tick_timer.stop();game.set_process(false);actor=game.bill_npc;game.powered_room_cells[HAB]=true
			...
			check(game.grid_view._get_human_frame(game.get_test_walker_state(),game.get_test_walker_direction()).get_image().get_data()==pixels,"Restore preserves pose pixels")
			...
			check(root.get_texture().get_image().save_png("res://output/crew-activity/bill-bunk/restore-%s-%04d.png"%["helmet" if equipped else "bare",roundi(elapsed*1000)])==OK,"Capture live restored contact")
			game.paused=false
			...
			check(game.grid_view._get_human_frame(game.get_test_walker_state(),game.get_test_walker_direction()).get_image().get_data()==pixels,"Reverse interruption preserves pose")
			check(actor.valid_bill_snapshot(actor.snapshot()),"Interrupted rise validates")
	...
	var bad=initial.duplicate(true);bad.bill_bunk.rest=Vector2(INF,0)
	check(not actor.valid_bill_snapshot(bad),"Nonfinite contact rejected")
	bad=initial.duplicate(true);bad.bill_bunk.entry+=Vector2(5,0)
	check(not actor.valid_bill_snapshot(bad),"Detached saved entry rejected")
	bad=initial.duplicate(true);bad.direction=false
	check(not actor.valid_bill_snapshot(bad),"Malformed saved direction rejected")
	bad=initial.duplicate(true);bad.timer=1.85
	check(not actor.valid_bill_snapshot(bad),"Excess transition timer rejected")
	...
	print("BILL BUNK: normal chooser/arrival, fourteen bare/equipped disk restores, pause, reverse interruption, completion and invalid saves; %d failures"%failures)
	quit(failures)
