extends SceneTree
const View=preload("res://rooms/underwater/batch-two/med_center_view.gd")
var destination:=""
class MedCenterPanel extends Node2D:
	var view
	func _draw() -> void:
			...
			for frame in range(2):
				...
				for i in range(view.props.size()):
					...
					if frame==0: before.append(pixels)
						...
						push_error("MedCenter host motion mismatch: "+str(view.props[i].id))
						failures+=1
	if failures==0: print("MED CENTER PILOT PASS: four rotations, four host motion/offline comparisons; direct renderer state, not economy integration")
