extends SceneTree
const OUT="res://output/reactor-operating-feedback-2026-09-21"
class Preview extends Node2D:
	...
	func _draw()->void:
		...
		preload("res://rooms/whole-room/north_wall.gd").draw_into(self,"reactor",Vector2i.ZERO,false,false,room)
...
func run()->void:
	...
	var room=load("res://rooms/whole-room/reactor_view.gd").new()
	...
	for q in range(4):
		...
		var props=room.props.filter(func(p):return p.id=="library/tileset-npp2-107")
		check(props.size()==1,"Installed console present q%d"%q)
		if props.size()!=1:continue
		var prop=props[0];var factor=prop.rect.size.x/prop.registration.width
		var origin=Vector2(prop.rect.get_center().x,prop.rect.end.y)+(Vector2(200,701)-prop.registration.pivot)*factor
		var allowed=Rect2(Vector2(256,290)+origin*1.06,Vector2(26,12)*factor*1.06).grow(3)
		var changed=0;var escaped=0
		for y in range(512):
			for x in range(512):
				if shots[2].get_pixel(x,y)!=shots[3].get_pixel(x,y):
					changed+=1
					if not allowed.has_point(Vector2(x,y)):escaped+=1
		check(changed>0 and escaped==0,"Motion must stay inside screen q%d changed=%d escaped=%d"%[q,changed,escaped])
	print("REACTOR CONSOLE FEEDBACK: four views; operating motion, off-state and held-clock checks; %d failures"%failures)
