...
func _test_blueprint_reward_unlocks_and_becomes_next_draw() -> void:
	...
	var data_before: int = int(game.meta.total_research_points)
	game._award_synergy_stabilization(_test_link("closed_air_loop"))
	# Owner playtest, Sept 17: blueprints are bought with Archived Data, not decrypted.
	_expect_true(not game.meta.unlocked_room_ids.has("biodome"), "stabilization no longer unlocks a blueprint")
	_expect_equal(game.draw_pile.count("biodome"), 0, "stabilization adds no prototype card")
	_expect_equal(game.run_decrypted_blueprint_ids, [], "nothing is decrypted")
	_expect_equal(game.meta.total_research_points, data_before + preload("res://scripts/meta_shop.gd").STABILIZE_DATA, "stabilization pays Archived Data")
	game.free()
	_remove_test_save(save_path)
...
func _test_terminal_stabilization_grants_research_once() -> void:
	...
	game._award_synergy_stabilization(_test_link("drone_foundry"))
	_expect_equal(game.meta.total_research_points, research_before + 3 + preload("res://scripts/meta_shop.gd").STABILIZE_DATA, "terminal Archived Data should be awarded exactly once")
	game.free()
	_remove_test_save(save_path)
...
func _test_three_functioning_cycles_complete_the_clean_unlock_chain() -> void:
	...
	_expect_true(not game.meta.unlocked_room_ids.has("biodome"), "Biodome is bought in Meta Progression, not decrypted")
	_expect_true(not game.draw_pile.has("biodome"), "the current run gets no Biodome prototype")
