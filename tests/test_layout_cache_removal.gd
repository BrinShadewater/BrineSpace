...
func run():
	Store.loaded=true
	Store.data={"emergency-isolation-wall/0":{"full_wall_emergency-isolation-wall":null,"library/tileset-npp2-62":[-96,16.56],"library/tileset-mtp2-91":[-174,6]}}
	Store.prime()
	...
	for empty in [false,true]:
		...
		else:room.props=room.props.filter(func(prop):return prop.id!="library/tileset-npp2-62")
		check(Store.apply(room,"emergency-isolation-wall"),"Removed props must invalidate cached application")
		check(room.props.filter(func(prop):return prop.id=="library/tileset-npp2-62").size()==1,"Saved cabinet must be restored once")
		check(room.props.filter(func(prop):return prop.id=="library/tileset-mtp2-91").size()==1,"Saved battery must remain or be restored once")
	...
	Store.data["emergency-isolation-wall/0"]["library/tileset-npp2-62"]=null
	Store.revision+=1
	Store.apply(room,"emergency-isolation-wall")
	check(room.props.filter(func(prop):return prop.id=="library/tileset-npp2-62").is_empty(),"Explicit saved deletion must stay deleted")
