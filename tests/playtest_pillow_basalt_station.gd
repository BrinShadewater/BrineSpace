extends SceneTree
const View := preload("res://assets/environment/pillow-basalt-v1/pillow_basalt_view.gd")
const OUT := "res://output/pillow-basalt-station-v1/"
var game
...
class Sheet extends Control:
	...
	func _draw() -> void:
		...
		draw_string(font,Vector2(30,44),"BRINE / PILLOW BASALT",HORIZONTAL_ALIGNMENT_LEFT,-1,26,Color("b7cec5"))
		...
		for id in View.SOURCES:
			...
			i += 1
		draw_string(font,Vector2(30,780),"Pillow basalt source sheet. Decorative ground and debris; no resource or clearance mechanics.",HORIZONTAL_ALIGNMENT_LEFT,-1,20,Color("839f9d"))
...
func run() -> void:
	...
	game.meta.save_path = "user://pillow_basalt_%d.meta" % OS.get_process_id()
	...
	assert(game.occupied==occupancy and game.resources==resources,"Rendering pillow_basalt changes no gameplay state")
	assert(game.grid_view.seabed_background.pillow_basalt.textures.size()==1)
	...
	print("PILLOW BASALT STATION PASS: one texture, three verified resolutions, unchanged occupancy and resources")
	quit()
