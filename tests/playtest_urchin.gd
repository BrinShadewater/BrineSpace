...
class Sheet extends Control:
	...
	func _draw() -> void:
		...
		var names := ["Urchin","Seagrass","Ribbon kelp"]
		for i in range(textures.size()):
			...
			for row in range(2):
				...
				for j in range(3):
					var side: float = [18.0,24.0,30.0][j]
		...
		draw_string(ThemeDB.fallback_font,Vector2(40,875),"Scale comparison: 120 pixels per room cell; 0.15 / 0.20 / 0.25 cell widths. No live integration implied.",HORIZONTAL_ALIGNMENT_LEFT,-1,20)
...
func run() -> void:
	...
	for path in ["res://assets/environment/urchin-v1/short-spine-urchin-v1.png","res://assets/environment/low-growth-v1/seagrass-rosette-v2.png","res://assets/environment/sub-biomes-v1/kelp-ribbon-kelp-v1.png"]:
		var source := Image.new()
		if source.load(path) != OK: push_error("Failed to load image (tests/playtest_urchin.gd:31)")
	...
	assert(frame.save_png("res://output/urchin-v1/organism-scale.png")==OK)
	print("URCHIN STUDY PASS: three alpha sources; three cell widths; source and runtime tint; 1600x900")
	quit()
