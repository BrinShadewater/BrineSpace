"""Compare native captures after running test_brine_batch_parity.gd."""
...
for state in ('q0', 'q1', 'q2', 'q3', 'close', 'rotate-live', 'power-on', 'power-off', 'light-fade', 'raised-walls', 'place', 'remove', 'door-open', 'door-close', 'cull', 'pan', 'zoom') + tuple(f'motion-{q}-{tick}' for q in range(4) for tick in range(3)) + tuple(f'busy-{frame}' for frame in (19,59,99,159)):
    direct = Image.open(root / f'brine-batch-parity-{state}-direct.png').convert('RGBA')
    cached = Image.open(root / f'brine-batch-parity-{state}-cached.png').convert('RGBA')
...
(root / 'brine-batch-pixel-comparison.json').write_text(json.dumps(records, indent=2), encoding='utf-8')
assert all(record['pixel_identical'] for record in records), records
print('BRINE BATCH PIXEL PARITY PASS: all four channels identical in all captures')
