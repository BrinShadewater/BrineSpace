extends SceneTree
const OUT := "res://output/station-composition-v1/"
var game
...
func run() -> void:
	...
	preload("res://scripts/room_layout_store.gd").path="res://output/station-composition-v1/isolated-layouts.json"
	...
	for i in range(0,ids.size(),3):
		game.inspector_focus_button.set_meta("cell",cells[i])
		game._focus_inspected_room()
		await capture("neighbours-"+str(i))
