extends SceneTree
const View := preload("res://assets/environment/shell-shoal-v1/shell_shoal_view.gd")
const OUT := "res://output/shell-shoal-station-v1/"
var game
...
class Sheet extends Control:
	...
	func _draw() -> void:
		...
		draw_string(font,Vector2(30,44),"BRINE / SHELL SHOAL",HORIZONTAL_ALIGNMENT_LEFT,-1,26,Color("b7cec5"))
		...
		for id in View.SOURCES:
			...
			i += 1
		draw_string(font,Vector2(30,780),"Shell shoal source sheet. Decorative ground and debris; no resource or clearance mechanics.",HORIZONTAL_ALIGNMENT_LEFT,-1,20,Color("839f9d"))
...
func run() -> void:
	...
	game.meta.save_path = "user://shell_shoal_%d.meta" % OS.get_process_id()
	game.run_save_path = "user://shell_shoal_%d.loop" % OS.get_process_id()
	root.add_child(game)
	current_scene = game
	...
	assert(game.occupied==occupancy and game.resources==resources,"Rendering shell shoal changes no gameplay state")
	assert(game.grid_view.seabed_background.shell_shoal.textures.size()==3)
	...
	print("SHELL SHOAL STATION PASS: three textures, three verified resolutions, unchanged occupancy and resources")
	quit()
