...
func run() -> void:
	...
	await settle()
	DirAccess.make_dir_recursive_absolute("res://output/hab-rug-v1")
	for q in range(4):
		var width := 1600
		...
		for room_id in ROOMS:
			...
			assert(frame.save_png("res://output/hab-rug-v1/%s-%d.png" % [room_id,q])==OK)
	...
	print("ROOM FLOORS PASS: green hab rug at four rotations")
	quit()
