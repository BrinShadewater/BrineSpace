...
class Sample extends Node2D:
	...
	var width:=360.0
	func _draw():
		Art._draw_articulated(self,"salvage",Vector2(400,400),width,clock,Color.WHITE)
...
func run():
	...
	for width in [180.0,360.0]:
		...
		var body_rect:=Rect2i(origin+Vector2(140,200)*factor,Vector2(80,70)*factor)
		var claw_rect:=Rect2i(origin+Vector2(0,285)*factor,Vector2(360,100)*factor)
		...
		for phase in range(8):
			...
			if phase==0:body=next_body;claws=next_claws
				...
				if next_body!=body:failures+=1;push_error("Salvage chassis shears at phase %d width %s"%[phase,width])
				if next_claws!=claws:moved=true
		if not moved:failures+=1;push_error("Salvage claws are frozen")
	print("SALVAGE ARTICULATION: 16 native poses, %d failures"%failures)
	quit(failures)
