"""Register sample handling to the corrected west kneeling endpoint."""
...
def main():
    source = BASE / 'sources/sample-west-body-01.png'
    raw = binary(chroma(source))
    assert raw.size == (2172, 724)
    endpoint = Image.open(BASE / 'review/west-kneel-body-01/kneel-west-005.png').convert('RGBA')
    scale = 106 / 499
    ...
    for i in range(4):
        ...
        offset = (round(128 - support * scale), round(222 - sole * scale))
    ...
    for row, action in enumerate(['kneel', 'repair', 'stand']):
        for i in range(6):
            if action == 'repair':
                frame = frames[i]
                frame.save(out / f'repair-west-{i:03}.png')
            else:
                frame = Image.open(out / f'{action}-west-{i:03}.png').convert('RGBA')
    ...
    print('Prepared west sample action joined to corrected kneeling endpoint.')
