...
func run() -> void:
	if capture_dir.is_empty(): capture_dir="res://output/salvage-owner-repair-2026-09-12/station-pause"
	DirAccess.make_dir_recursive_absolute(capture_dir)
	preload("res://scripts/title_settings.gd").save_path="user://salvage_asset_pause.cfg"
	game=MainScene.instantiate()
	game.meta.save_path="user://salvage_asset_pause.meta"
	game.run_save_path="user://salvage_asset_pause.loop"
	...
	game._place_room("salvage_workshop",cell,true)
	...
	for q in range(4):
		...
		var salvage_view=game.grid_view.rare_room_views["salvage_workshop"]
		var machine: float=salvage_view.machine_clock
		...
		expect(salvage_view.machine_clock==machine,"Paused Salvage renderer clock frozen")
		expect(salvage_view.operating,"Salvage is powered during pause review")
		expect(frozen==room_pixels(cell).get_data(),"Paused Salvage native pixels frozen")
		records.append({"quarter":q,"clock_before_run":start,"paused_clock":clock,"machine_clock":machine,"operating":salvage_view.operating,"pixels_equal":frozen==room_pixels(cell).get_data()})
	...
	print("SALVAGE STATION PAUSE: failures=",failures)
