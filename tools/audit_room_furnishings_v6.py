"""Audit the six v6 sources, registrations, cards, profiles, and native views."""
...
MANIFEST = ROOT / "assets/room-furnishings-v6/manifest.json"
...
def main() -> None:
    ...
    summary = {"assets": len(data["assets"]), "themed": sum(item["scope"] == "themed" for item in data["assets"]), "universal": sum(item["scope"] == "universal" for item in data["assets"]), "large": sum(item["size_class"] == "large" for item in data["assets"]), "medium": sum(item["size_class"] == "medium" for item in data["assets"]), "rooms": len({item["room"] for item in data["assets"]}), "native_orientations": sum(len(value) for value in views.values()), "errors": errors}
