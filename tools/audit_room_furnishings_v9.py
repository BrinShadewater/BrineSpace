"""Audit the six v9 sources, registrations, cards, profiles, and native views."""
...
MANIFEST = ROOT / "assets/room-furnishings-v9/manifest.json"
...
def main() -> None:
    ...
    summary = {
        ...
        "themed": sum(x["scope"] == "themed" for x in data["assets"]),
        "universal": sum(x["scope"] == "universal" for x in data["assets"]),
        "large": sum(x["size_class"] == "large" for x in data["assets"]),
        "medium": sum(x["size_class"] == "medium" for x in data["assets"]),
        "rooms": len({x["room"] for x in data["assets"]}),
        "native_orientations": sum(len(v) for v in views.values()),
        "errors": errors,
    }
