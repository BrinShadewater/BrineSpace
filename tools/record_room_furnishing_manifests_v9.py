"""Record v9 dependencies in the owning room manifests."""
...
UPDATES = {
    "radio_lab": ("rooms/underwater/acoustic-comms/manifest.json", "rooms/underwater/acoustic-comms/radio-composition-v5.json"),
    "listening_post": ("rooms/underwater/rare-dead-ends/listening-manifest.json", "rooms/underwater/rare-dead-ends/listening-composition-v3.json"),
    "holographic_core": ("rooms/underwater/batch-two/export-manifest.json", "rooms/underwater/batch-two/holo-composition-v5.json"),
}
...
def main() -> None:
    ...
    for manifest_path, changes in grouped.items():
        ...
        for room, profile_path in changes:
            ...
            card = f"assets/room-furnishings-v9/cards/{room}.png"
            record.setdefault("integration", {})["card"] = card
            record["card"] = {"path": card, "sha256": digest(card)}
            record.setdefault("verification", {})["furnishing_v9"] = "Native four-orientation review and route evidence recorded in assets/room-furnishings-v9/manifest.json; owner acceptance pending."
