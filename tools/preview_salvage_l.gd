...
class Composition extends Node2D:
	...
	func _draw() -> void:
		...
		var source_region:=Rect2(955,488,291,605)
...
func run() -> void:
	...
	assert(source.load_png_from_buffer(FileAccess.get_file_as_bytes("res://rooms/underwater/l-shaped-studies/salvage-clean-v2.png"))==OK)
	...
	assert(straight_image.load_png_from_buffer(FileAccess.get_file_as_bytes("res://rooms/underwater/l-shaped-studies/salvage-straight-clean-v1.png"))==OK)
	...
	assert(root.get_texture().get_image().save_png("res://rooms/underwater/l-shaped-studies/salvage-room-v3.png")==OK)
	print("SALVAGE L ROOM COMPOSITION PASS")
	quit()
