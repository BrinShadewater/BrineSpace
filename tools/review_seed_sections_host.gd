...
const OUT := "res://output/seed-sections-host-study"
...
class Board extends Node2D:
	...
	func _draw() -> void:
		...
		for i in range(2):
			...
			draw_string(ThemeDB.fallback_font,Vector2(35+i*500,535),"Closed west wall" if i==0 else "Open west wall: 96-unit section gap",HORIZONTAL_ALIGNMENT_LEFT,-1,17)
...
func run() -> void:
	...
	for i in range(2):
		...
		reports.append({"open_sides":[] if i==0 else [3],"sections":overlaps})
