"""Register Veld south scanner interiors with independent original endpoints."""
...
def main(direction="south"):
    ...
    durations=[140]*6
    endpoint=BASE/'sources/veld-role-interact-south-end-reference-01.png'
    source=BASE/f'sources/veld-role-interact-{direction}-strip-01.png'
    raw=binary(chroma(source))
    expected,height,soles=(1681,936),607,[739]*4
    assert raw.size==expected
    scale=148/height
    frames=[Image.open(BASE/f'sources/veld-role-interact-{direction}-reference-01.png').convert('RGBA')]
    supports=[]
    for i in range(4):
