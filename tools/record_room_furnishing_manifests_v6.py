"""Record v6 profile, card, and component dependencies in owning room manifests."""
...
UPDATES = {
    "tidal_condenser": ("rooms/underwater/tidal-condenser/manifest.json", "rooms/underwater/tidal-condenser/tidal-composition-v3.json"),
    "gravity_loom": ("rooms/underwater/gravity-loom/manifest.json", "rooms/underwater/gravity-loom/loom-composition-v3.json"),
    "xeno_lab": ("rooms/underwater/batch-two/export-manifest.json", "rooms/underwater/batch-two/xeno-composition-v3.json"),
}
...
def main() -> None:
    ...
    for manifest_path, changes in grouped.items():
        ...
        for room, profile_path in changes:
            ...
            card = f"assets/room-furnishings-v6/cards/{room}.png"
            if isinstance(record.get("integration"), dict): record["integration"]["card"] = card
            record["card"] = {"path": card, "sha256": digest(card)}
            record.setdefault("verification", {})["furnishing_v6"] = "Native four-orientation review and route evidence recorded in assets/room-furnishings-v6/manifest.json; owner acceptance pending."
