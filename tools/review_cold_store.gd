...
func run() -> void:
	...
	DirAccess.make_dir_recursive_absolute("res://output/cold-store-v1")
	var room:=View.new()
	room.embedded=true
	room.hide()
	root.add_child(room)
	...
	assert(root.get_texture().get_image().save_png("res://legacy/default/rooms/underwater/cold-store-v1/card.png")==OK)
	canvas.open=[0,2]
	canvas.running=true
	canvas.queue_redraw()
	await process_frame
	await RenderingServer.frame_post_draw
	assert(root.get_texture().get_image().save_png("res://output/cold-store-v1/open.png")==OK)
	...
	for y in range(-188,189,4):
		assert(room.Geometry.can_stand(Vector2(0,y),room.layout,room.props,room.edges),"South approach blocked")
	assert(room.Geometry.has_port(room.layout[0],0))
	assert(room.Geometry.has_port(room.layout[0],2))
	assert(not room.can_stand(Vector2(112,0)),"Bookshelf lacks collision")
	print("COLD STORE ART PASS: native card, two registered pieces, clear south approach and blocked shelves")
	for path in ["res://tools/review_cold_store.gd","res://rooms/underwater/cold-store-v1/cold_store_view.gd"]:
