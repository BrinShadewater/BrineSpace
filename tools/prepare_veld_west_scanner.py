"""Register Veld west scanner interiors with independent original endpoints."""
...
def main(direction="west"):
    ...
    durations=[140]*6
    endpoint=BASE/'sources/veld-west-scanner-body-closing-01.png'
    source=BASE/'sources/veld-west-scanner-strip-01.png'
    raw=binary(chroma(source))
    expected,height,soles=(1774,887),584,[744,745,744,744]
    assert raw.size==expected
    scale=148/height
    frames=[Image.open(BASE/f'sources/veld-west-scanner-body-opening-01.png').convert('RGBA')]
    supports=[]
    for i in range(4):
