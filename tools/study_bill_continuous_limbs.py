...
OUT = ROOT / 'output/bill-pixel-layer-2026-09-21/isolated-limbs'
    ...
    for direction, anchors in ANCHORS.items():
        ...
        for phase,poses in enumerate(trajectory):
            ...
            for leg in ('far','near'):
                ...
                limb=mesh_limb(isolated[leg],(hip,knee,ankle),(th,tk,ta))
                limb.paste((0,0,0,0),(0,0,184,119+bob))
        ...
        audit[direction]={'source_sha256':hashlib.sha256(source_path.read_bytes()).hexdigest(),
                          'trajectory':trajectory,'source_anchors':anchors,'scope':'Refined source knees, shared-edge surfaces, preserved hip/ankle paths and foot layers. No torso/arm repair or runtime selection change.'}
