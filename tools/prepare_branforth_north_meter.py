"""Register Branforth north scanner interiors with independent original endpoints."""
...
def main(direction="north"):
    ...
    durations=[140]*6
    endpoint=BASE/'sources/branforth-north-meter-body-closing-01.png'
    source=BASE/f'sources/branforth-north-meter-strip-01.png'
    raw=binary(chroma(source))
    expected,height,soles=(1983,793),570,[679]*4
    assert raw.size==expected
    scale=148/height
    frames=[Image.open(BASE/f'sources/branforth-north-meter-body-opening-01.png').convert('RGBA')]
    supports=[]
    for i in range(4):
