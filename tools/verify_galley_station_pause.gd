...
func run() -> void:
	if capture_dir.is_empty(): capture_dir="res://output/galley-owner-repair-2026-09-12/station-pause"
	DirAccess.make_dir_recursive_absolute(capture_dir)
	preload("res://scripts/title_settings.gd").save_path="user://galley_asset_pause.cfg"
	game=MainScene.instantiate()
	game.meta.save_path="user://galley_asset_pause.meta"
	game.run_save_path="user://galley_asset_pause.loop"
	...
	game._place_room("galley",cell,true)
	...
	for q in range(4):
		...
		var galley_view=game.grid_view.rare_room_views["galley"]
		var machine: float=galley_view.machine_clock
		...
		expect(galley_view.machine_clock==machine,"Paused Galley renderer clock frozen")
		expect(galley_view.operating,"Galley is powered during pause review")
		expect(frozen==room_pixels(cell).get_data(),"Paused Galley native pixels frozen")
		records.append({"quarter":q,"clock_before_run":start,"paused_clock":clock,"machine_clock":machine,"operating":galley_view.operating,"pixels_equal":frozen==room_pixels(cell).get_data()})
	...
	print("GALLEY STATION PAUSE: failures=",failures)
