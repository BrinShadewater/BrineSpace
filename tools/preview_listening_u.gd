...
func run() -> void:
	...
	var room=load("res://rooms/underwater/rare-dead-ends/listening_post_view.gd").new()
	room.embedded=true
	room.hide()
	root.add_child(room)
	var source:=Image.new()
	assert(source.load_png_from_buffer(FileAccess.get_file_as_bytes("res://rooms/underwater/rare-dead-ends/listening-u-flush-clean-v1.png"))==OK)
	...
	assert(root.get_texture().get_image().save_png("res://rooms/underwater/rare-dead-ends/listening-u-flush-room-v1.png")==OK)
