"""Register authored north scanner bodies against corrected Veld idle anatomy."""
...
def main():
    source = BASE / 'sources/scanner-north-body-01.png'
    raw = binary(chroma(source))
    assert raw.size == (2043, 770)
    idle = Image.open(BASE / 'review/directional-movement-01/idle-north-000.png').convert('RGBA')
    scale = 143 / 576
    frames = [idle]
    records = []
    for i in range(4):
    ...
    for i, frame in enumerate(frames):
        frame.save(out / f'interact-north-{i:03}.png')
    ...
    print('Registered four north body interiors and exact corrected idle endpoints.')
