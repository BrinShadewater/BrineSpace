...
def main():
    source=BASE/'sources/branforth-north-helmet-heads-01.png'
    raw=binary(chroma(source));assert raw.size==(2170,725)
    out=BASE/'review/branforth-role-interact-north-strip-01'
    frames=[Image.open(BASE/'sources/branforth-north-meter-equipment-opening-01.png').convert('RGBA')]
    for i in range(4):
        ...
        head=raw.crop((i*raw.width//4+148,173,i*raw.width//4+397,426))
        head=binary(head.resize((38,36),Image.Resampling.BOX),True)
        fitted=body.copy()
    ...
    frames.append(Image.open(BASE/'sources/branforth-north-meter-equipment-closing-01.png').convert('RGBA'))
    ...
    report=dict(status='unselected_head_fit_study',source=source.relative_to(ROOT).as_posix(),sha256=hashlib.sha256(source.read_bytes()).hexdigest(),headSize=[38,36],position=[73,20],bodyPixelsPreservedBelowY=60,limits='Head seam and paired native visual review pending; generated shoulders excluded.')
