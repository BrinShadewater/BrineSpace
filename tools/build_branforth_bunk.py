"""Install the locally reproducible Branforth bunk supplement into his selected catalog."""
...
def build():
    for script in ['build_branforth_entry.py','build_branforth_equipment.py']:
        runpy.run_path(str(SOURCE/script),run_name='__main__')
    revision=ROOT/'character/chief-engineer-branforth-v2'
    catalog=json.loads((revision/'catalog.json').read_text())
    for group,source,variant in [('body','branforth-entry','bare'),('equipment','branforth-entry-helmet','helmet')]:
