"""Install the locally reproducible Veld bunk supplement into her selected catalog."""
...
def build():
    for script in ['build_lowering.py','build_entry.py','build_equipment.py']:
        runpy.run_path(str(SOURCE/script),run_name='__main__')
    revision=ROOT/'character/dr-veld-v2'
    catalog=json.loads((revision/'catalog.json').read_text())
    for group,source,variant in [('body','veld-entry','bare'),('equipment','veld-entry-helmet','helmet')]:
