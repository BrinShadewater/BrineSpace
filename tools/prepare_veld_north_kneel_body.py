"""Prepare the female Veld north kneeling transition at one anatomical scale."""
...
def main():
    source = BASE / 'sources/kneel-north-body-01.png'
    raw = binary(chroma(source))
    assert raw.size == (2172, 724)
    scale = 143 / 548  # Source standing pose is the common anatomical ruler.
    frames, records = [], []
    ...
    for i, (start, end) in enumerate(zip(starts, ends)):
        ...
        # Rear boot rises toward the viewer; the planted left boot owns ground registration.
        alpha = np.asarray(tile)[:, :, 3]
        mid = round((box[0] + box[2]) / 2)
        ys, _ = np.where(alpha[:, :mid] > 0)
        sole = int(ys.max())
        ...
        offset = (round(112 - support * scale), round(223 - sole * scale))
    ...
    frames[0] = Image.open(BASE / 'review/directional-movement-01/idle-north-000.png').convert('RGBA')
    ...
    for row, (action, poses) in enumerate([('kneel', frames), ('stand', list(reversed(frames)))]):
        for i, frame in enumerate(poses):
            frame.save(out / f'{action}-north-{i:03}.png')
    ...
    print('Prepared north kneel/stand chain with one body ruler.')
