...
func run() -> void:
	if capture_dir.is_empty(): capture_dir="res://output/cold-store-owner-repair-2026-09-12/station-pause"
	DirAccess.make_dir_recursive_absolute(capture_dir)
	preload("res://scripts/title_settings.gd").save_path="user://cold_store_asset_pause.cfg"
	game=MainScene.instantiate()
	game.meta.save_path="user://cold_store_asset_pause.meta"
	game.run_save_path="user://cold_store_asset_pause.loop"
	...
	game._place_room("cold_store",cell,true)
	...
	for q in range(4):
		...
		var cold_store_view=game.grid_view.rare_room_views["cold_store"]
		var machine: float=cold_store_view.machine_clock
		...
		expect(cold_store_view.machine_clock==machine,"Paused Cold Store renderer clock frozen")
		expect(cold_store_view.operating,"Cold Store is powered during pause review")
		expect(frozen==room_pixels(cell).get_data(),"Paused Cold Store native pixels frozen")
		records.append({"quarter":q,"clock_before_run":start,"paused_clock":clock,"machine_clock":machine,"operating":cold_store_view.operating,"pixels_equal":frozen==room_pixels(cell).get_data()})
	...
	print("COLD STORE STATION PAUSE: failures=",failures)
