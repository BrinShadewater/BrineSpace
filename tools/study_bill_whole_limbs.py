...
OUT = ROOT / 'output/bill-pixel-layer-2026-09-21/whole-limbs'
    ...
    for direction, anchors in ANCHORS.items():
        ...
        for phase,poses in enumerate(trajectory):
            ...
            for leg in ('far','near'):
                ...
                limb=place(isolated[leg],ankle,hip,ta,th)
                bounds=limb.getbbox()
        ...
        audit[direction]={'source_sha256':hashlib.sha256(source_path.read_bytes()).hexdigest(),
                          'trajectory':trajectory,'source_anchors':anchors,'scope':'Whole authored leg including boot, rigid source rotation with ankle anchor and sole-height correction. Preserved upper-body poses with fixed head; no runtime selection change.'}
