...
func run() -> void:
	...
	var folder="res://output/cold-store-owner-repair-2026-09-12/frost-states-v2"
	...
	for powered in [false,true]:
		for i in range(previews.size()):
			var room=previews[i].room;room.operating=powered;room.machine_clock=1.0
			var queue=[]
	...
	for path in ["res://tools/capture_cold_store_frost_states.gd"]:
		if not FileAccess.file_exists(path+".uid"):
			var uid_file:=FileAccess.open(path+".uid",FileAccess.WRITE)
			uid_file.store_line(ResourceUID.id_to_text(ResourceUID.create_id()))
			uid_file.close()
	print("COLD STORE FROST STATES: 4 rotations x off/on/two clocks, direct, retained and fixed-clock cache transitions")
	quit()
