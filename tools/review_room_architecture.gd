...
class Preview extends Node2D:
	...
	var q:=0
	func _draw() -> void:
		draw_rect(Rect2(0,0,760,850),Color("12282e"))
		draw_set_transform(Vector2(380,380),0,Vector2.ONE*1.5)
		...
		room.render_into(self,Vector2(380,380),1.5,true)
		draw_set_transform(Vector2(380,380),0,Vector2.ONE*1.5)
		preload("res://rooms/whole-room/north_wall.gd").draw_into(self,id,Vector2i.ZERO,false,false,room)
		room.render_into(self,Vector2(380,380),1.5,false,false)
		draw_string(ThemeDB.fallback_font,Vector2(50,825),id.replace("_"," ").capitalize(),HORIZONTAL_ALIGNMENT_LEFT,-1,26,Color("c1cec7"))
func _init() -> void: call_deferred("run")
func run() -> void:
	root.size=Vector2i(760,850)
	...
	DirAccess.make_dir_recursive_absolute("res://output/room-architecture-v4")
	var manifest: Array=[]
	...
	for entry in manifest:
		...
		for q in range(4):
			...
			assert(root.get_texture().get_image().save_png("res://output/room-architecture-v4/%s-q%d.png"%[entry.room,q])==OK)
