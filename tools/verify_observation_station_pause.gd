...
func run() -> void:
	if capture_dir.is_empty(): capture_dir="res://output/observation-owner-repair-2026-09-12/station-pause"
	DirAccess.make_dir_recursive_absolute(capture_dir)
	preload("res://scripts/title_settings.gd").save_path="user://observation_asset_pause.cfg"
	game=MainScene.instantiate()
	game.meta.save_path="user://observation_asset_pause.meta"
	game.run_save_path="user://observation_asset_pause.loop"
	...
	game._place_room("observation_room",cell,true)
	...
	for q in range(4):
		...
		var observation_view=game.grid_view.rare_room_views["observation_room"]
		var machine: float=observation_view.machine_clock
		...
		expect(observation_view.machine_clock==machine,"Paused Observation renderer clock frozen")
		expect(observation_view.operating,"Observation is powered during pause review")
		expect(frozen==room_pixels(cell).get_data(),"Paused Observation native pixels frozen")
		records.append({"quarter":q,"clock_before_run":start,"paused_clock":clock,"machine_clock":machine,"operating":observation_view.operating,"pixels_equal":frozen==room_pixels(cell).get_data()})
	...
	print("OBSERVATION STATION PAUSE: failures=",failures)
