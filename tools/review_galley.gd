...
func run() -> void:
	...
	DirAccess.make_dir_recursive_absolute("res://output/galley-v1")
	var room:=View.new()
	room.embedded=true
	room.hide()
	root.add_child(room)
	...
	assert(root.get_texture().get_image().save_png("res://legacy/default/rooms/underwater/galley-v1/card.png")==OK)
	canvas.open=[2]
	canvas.running=true
	canvas.queue_redraw()
	await process_frame
	await RenderingServer.frame_post_draw
	assert(root.get_texture().get_image().save_png("res://output/galley-v1/open.png")==OK)
	...
	for y in range(0,161,4):
		assert(room.Geometry.can_stand(Vector2(-40,y),room.layout,room.props,room.edges),"South approach blocked")
	assert(not room.Geometry.has_port(room.layout[0],0))
	assert(room.Geometry.has_port(room.layout[0],2))
	assert(not room.can_stand(Vector2(70,96)),"Bookshelf lacks collision")
	print("GALLEY ART PASS: native card, two registered pieces, clear south approach and blocked shelves")
	for path in ["res://tools/review_galley.gd","res://rooms/underwater/galley-v1/galley_view.gd"]:
