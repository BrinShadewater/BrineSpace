...
from build_marsh_carry_turns import ROOT, BASE, build as adjacent
...
def build(install=False,pair='east-west'):
    ...
    a=adjacent(pair=first)[f'carry-turn-{origin}-{via}']
    b=adjacent(pair=second)[f'carry-turn-{via}-{destination}']
    ...
    clips={f'carry-turn-{pair}':row,f'carry-turn-{destination}-{origin}':list(reversed(row))}
    ...
    for key,frames in clips.items():
        ...
        states.append(dict(id=key,frameFiles=files,frameDurationsMs=durations,loop=False,
            ...
            waterKinds=['carry']*9,waterPoses=[False]*9,depthOffsets=[0.0]*9))
