...
func run() -> void:
	...
	for row in range(3):
		for q in range(4):
			...
			room.actor=room.Geometry.turn(Vector2(-78,45),q)
	...
	var folder="res://output/cold-store-owner-repair-2026-09-12/crew-scale"
	DirAccess.make_dir_recursive_absolute(folder)
	assert(root.get_texture().get_image().save_png(folder+"/direct.png")==OK)
	print("COLD STORE CREW SCALE: production Bill reference, four orientations")
	quit()
