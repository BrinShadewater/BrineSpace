...
OUT = ROOT / 'output/bill-pixel-layer-2026-09-21/ankle-blend'
...
def ankle_blend(source,ankle,hip,ta,th,foot_angle):
    u=hip-ankle;v=th-ta
    leg_angle=math.atan2(v[1],v[0])-math.atan2(u[1],u[0])
    yy,xx=np.mgrid[:source.height,:source.width]
    t=np.clip((yy+.5-(ta[1]-10))/16,0,1);t=t*t*(3-2*t)
    angles=leg_angle*(1-t)+foot_angle*t
    dx=xx+.5-ta[0];dy=yy+.5-ta[1]
    sx=np.floor(dx*np.cos(angles)+dy*np.sin(angles)+ankle[0]).astype(int)
    sy=np.floor(-dx*np.sin(angles)+dy*np.cos(angles)+ankle[1]).astype(int)
    valid=(sx>=0)&(sy>=0)&(sx<source.width)&(sy<source.height)
    pixels=np.array(source);out=np.zeros_like(pixels)
    out[valid]=pixels[sy[valid],sx[valid]]
    ...
    for direction, anchors in ANCHORS.items():
        ...
        for phase,poses in enumerate(trajectory):
            ...
            for leg in ('far','near'):
                ...
                # Keep the entire authored leg/boot intact; anchor at the ankle.
                step=target['phase']
                angle=([-.25,-.25,-.25,-.1,-.3,-.25] if leg=='far' else [.25,.25,.25,.1,-.05,.1])[step]
                limb=ankle_blend(isolated[leg],ankle,hip,ta,th,angle)
                bounds=limb.getbbox()
                shift=round(target['soleTarget']-bounds[3])
        ...
        audit[direction]={'source_sha256':hashlib.sha256(source_path.read_bytes()).hexdigest(),
                          'trajectory':trajectory,'source_anchors':anchors,'scope':'Whole authored leg including boot, rigid upper-leg rotation blending locally to independent boot orientation, with sole-height correction. Preserved upper-body poses with fixed head; no runtime selection change.'}
