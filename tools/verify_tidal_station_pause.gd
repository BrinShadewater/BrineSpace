...
func run() -> void:
	capture_dir="res://output/tidal-owner-repair-2026-09-12/station-pause"
	DirAccess.make_dir_recursive_absolute(capture_dir)
	preload("res://scripts/title_settings.gd").save_path="user://tidal_asset_pause.cfg"
	game=MainScene.instantiate()
	game.meta.save_path="user://tidal_asset_pause.meta"
	game.run_save_path="user://tidal_asset_pause.loop"
	...
	game._place_room("tidal_condenser",cell,true)
	...
	for q in range(4):
		...
		var machine: float=game.grid_view.tidal_view.machine_clock
		...
		expect(game.grid_view.tidal_view.machine_clock==machine,"Paused Tidal renderer clock frozen")
		expect(game.grid_view.tidal_view.operating,"Tidal is powered during pause review")
		expect(frozen==room_pixels(cell).get_data(),"Paused Tidal native pixels frozen")
		records.append({"quarter":q,"clock_before_run":start,"paused_clock":clock,"machine_clock":machine,"operating":game.grid_view.tidal_view.operating,"pixels_equal":frozen==room_pixels(cell).get_data()})
	...
	print("TIDAL STATION PAUSE: failures=",failures)
