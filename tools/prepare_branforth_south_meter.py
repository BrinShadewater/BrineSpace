"""Register Branforth south scanner interiors with independent original endpoints."""
...
def main(direction="south"):
    ...
    durations=[140]*6
    endpoint=BASE/'sources/branforth-south-meter-body-closing-01.png'
    source=BASE/f'sources/branforth-south-meter-strip-01.png'
    raw=binary(chroma(source))
    expected,height,soles=(1962,801),577,[686]*4
    assert raw.size==expected
    scale=148/height
    frames=[Image.open(BASE/f'sources/branforth-south-meter-body-opening-01.png').convert('RGBA')]
    supports=[]
    for i in range(4):
