...
func run() -> void:
	...
	for id in ["refrigeration","pantry"]:
	...
	if not FileAccess.file_exists("res://assets/cold-store-wall-v1/render_assets.gd.uid"):
		var f:=FileAccess.open("res://assets/cold-store-wall-v1/render_assets.gd.uid",FileAccess.WRITE)
