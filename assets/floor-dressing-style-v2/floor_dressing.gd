...
static func catalog() -> Dictionary:
	if manifest.is_empty(): manifest=JSON.parse_string(FileAccess.get_file_as_string("res://assets/floor-dressing-style-v2/manifest.json"))
	return manifest.sprites
static func texture(id: String) -> Texture2D:
	if not textures.has(id):
		var image:=Image.new()
		if image.load_png_from_buffer(FileAccess.get_file_as_bytes("res://"+catalog()[id].path)) != OK: push_error("Failed to load image (assets/floor-dressing-style-v2/floor_dressing.gd:11)")
