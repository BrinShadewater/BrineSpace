extends SceneTree
const Kit=preload("res://assets/riser-wall-kit-style-v2/wall_sprites.gd")
func _init() -> void: call_deferred("run")
class Canvas extends Node2D:
	var layouts:=false
	func _draw() -> void:
		draw_rect(Rect2(0,0,1280,960),Color("172930"))
		draw_string(ThemeDB.fallback_font,Vector2(24,34),"BRINESPACE / OCEAN WINDOWS, HULL PANELS & SCREENS",HORIZONTAL_ALIGNMENT_LEFT,-1,22,Color("c8d1c3"))
		if layouts:
			var row:=0
...
func run() -> void:
	...
	if DisplayServer.get_name()!="headless":
		...
		for layout_mode in [false,true]:
			...
			root.get_texture().get_image().save_png("res://assets/riser-wall-kit-style-v2/"+("wall-layouts.png" if layout_mode else "preview.png"))
	print("RISER KIT PASS: 12 textures, native 1x/2x preview, 3 wall layouts, riser and hatch-bay bounds")
	quit()
