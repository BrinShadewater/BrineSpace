...
static func data() -> Dictionary:
	if manifest.is_empty(): manifest=JSON.parse_string(FileAccess.get_file_as_string("res://assets/wall-dressing-style-v1/manifest.json"))
	return manifest
static func catalog() -> Dictionary: return data().sprites
static func texture(id: String) -> Texture2D:
	if not textures.has(id):
		var image:=Image.new()
		if image.load_png_from_buffer(FileAccess.get_file_as_bytes("res://"+catalog()[id].path)) != OK: push_error("Failed to load image (assets/wall-dressing-style-v1/wall_sprites.gd:12)")
