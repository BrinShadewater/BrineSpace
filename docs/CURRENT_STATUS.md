...
Owner calls outstanding, all from the promoted layouts themselves rather than code:

1. `test_preferred_room_layouts` fails on `room-observation_room/2` with "blocked door 0" - that layout stands a prop in the north door lane.
2. Three rare-room rotations have no operating instrument, so they also have no crew station and no operating effects: Listening Post q0 and q2 (the layout deletes the instrument bank, previously masked by the reconfigure bug) and Pressure Control q0 (the layout deletes the flush panels, already the case on the owner's machine). `test_crew_room_activity` and `test_rare_operating_effects` fail on those rotations. Restoring the deleted wall in Listening Post q0 and q2 was measured to fix both (all four rotations back to 3 stations and 1 instrument); the owner chooses between keeping the deletions, restoring the instruments, or re-homing stations and effects onto the acoustic props they placed instead.

## Owner playtest notes, third fixes - September 15, 2026

Owner answers: 25% flicker threshold approved; sprinklers stay fire-only with no extra messaging; BRINE's room must flood; Margot should open a dialogue window like the others. Storage flow numbers and promoting the Studio layouts were explained and await an answer.
