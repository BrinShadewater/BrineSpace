...
func begin_life_station(station: Dictionary) -> void:
	bunk_motion.clear()
	if stage!="life_lie" or not station.get("bill_bunk",false):return
...
func snapshot() -> Dictionary:
	...
	result.bill_bunk=bunk_motion.duplicate(true) if bunk_motion_active() else {}
	return result

static func valid_bill_snapshot(data: Variant) -> bool:
	if not data is Dictionary:return false
	var motion: Variant=data.get("bill_bunk",{})
...
func restore_snapshot(main,data: Dictionary,staged := false) -> void:
	if not valid_bill_snapshot(data):return
	bunk_motion=data.get("bill_bunk",{}).duplicate(true)
