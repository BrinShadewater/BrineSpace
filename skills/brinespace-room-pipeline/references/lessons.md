...
Envelope-legality lesson: "outside the placement envelope" does not mean "reset in game". `RoomLayoutStore.apply()` only enforces the envelope for layouts saved in constrained mode; in free placement — the Studio default, and how most owner layouts are saved — it skips the guard for any prop still intersecting a generous box. A September 12 investigation raised a false alarm on a battery-array wall section by reading an envelope comparison as runtime behaviour; the placement was the owner's own and survived untouched. Before reporting that art is being reset, check `__free_placement` for that layout and confirm with a probe that the prop actually moves.
