extends SceneTree
const Kit=preload("res://archive/assets/wall-dressing-style-v1/wall_sprites.gd")
func _init() -> void: call_deferred("run")
class Canvas extends Node2D:
	var layouts:=false
	func _draw() -> void:
		draw_rect(Rect2(0,0,1280,1400),Color("172930"))
		draw_string(ThemeDB.fallback_font,Vector2(24,34),"BRINESPACE / WALL DECORATIONS / STYLE REVIEW",HORIZONTAL_ALIGNMENT_LEFT,-1,22,Color("c8d1c3"))
		if layouts:
			var row:=0
		...
		for id in Kit.catalog():
			var origin:=Vector2(14+(i%4)*316,64+(i/4)*219)
			draw_rect(Rect2(origin,Vector2(302,205)),Color("344a53"))
			draw_line(origin+Vector2(171,46),origin+Vector2(171,172),Color("263b44"),1)
			draw_string(ThemeDB.fallback_font,origin+Vector2(10,25),id,HORIZONTAL_ALIGNMENT_LEFT,-1,15,Color("d0d5c8"))
			Kit.draw(self,id,origin+Vector2(86,114),2)
			Kit.draw(self,id,origin+Vector2(236,114),1)
			draw_string(ThemeDB.fallback_font,origin+Vector2(72,189),"2x",HORIZONTAL_ALIGNMENT_LEFT,-1,12,Color("99afa7"))
			draw_string(ThemeDB.fallback_font,origin+Vector2(223,189),"1x",HORIZONTAL_ALIGNMENT_LEFT,-1,12,Color("99afa7"))
			i+=1
func run() -> void:
	assert(Kit.catalog().size()==24)
	for id in Kit.catalog():
	...
	if DisplayServer.get_name()!="headless":
		...
		for layout_mode in [false]:
			canvas.layouts=layout_mode
			canvas.queue_redraw()
			for frame in range(4): await process_frame
			await RenderingServer.frame_post_draw
			root.get_texture().get_image().save_png("res://assets/wall-dressing-style-v1/"+("wall-layouts.png" if layout_mode else "preview.png"))
	print("WALL DRESSING PASS: 24 textures, native 1x/2x style preview; room placement not tested")
	quit()
